...
**Symbol references (TS/JS):** alongside definition spans, `extractSymbolRefsFromFile` / `extractSymbolRefsJsTs` (`src/core/symbol-refs.ts`, `src/core/tree-sitter-refs-js.ts`) pull import, export, and call-site refs from the same grammars. Used as input to the repo-map graph (PageRank / `find_symbol` wiring follows); not yet persisted at sync time.
