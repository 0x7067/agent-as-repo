...
**Agentic search**: `ask` / `agent_call` expose live-repo tools (`grep_repo`, `glob_files`, `read_file`) alongside archival recall. Path safety lives in `src/core/repo-path.ts`; ripgrep argv building in `src/core/ripgrep-args.ts`; handlers in `src/shell/repo-tools.ts`. Persona prompts prefer exact navigation tools first and archival search for conceptual recall.
