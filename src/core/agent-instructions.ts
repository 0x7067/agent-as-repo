...
export function renderInstructionsBlock(input: RenderInstructionsBlockInput): string {
  return [
    ...
    "## Repo Expert (memory layer)",
    "",
    "This repository is indexed by repo-expert — local, continuously synced memory",
    "under your coding agent (MCP server: `repo-expert`). Use your host's",
    "grep/read/glob for exact identifiers and file navigation.",
    "",
    "Prefer these MCP tools for durable repo knowledge:",
    "- `agent_get_core_memory` — read architecture/conventions blocks first",
    "- `agent_search_archival` — hybrid semantic + lexical recall (optional `path_prefix` to narrow by directory)",
    "- `agent_call` — ask the expert a synthesized question",
    "",
