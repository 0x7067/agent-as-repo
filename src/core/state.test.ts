...
describe("state operations", () => {
  ...
  it("adds an agent to state", () => {
    const state = createEmptyState();
    const next = addAgentToState(state, "my-app", "agent-123", CREATED_AT);
  ...
  it("does not mutate original state", () => {
    const state = createEmptyState();
    addAgentToState(state, "my-app", "agent-123", CREATED_AT);
    expect(state.agents).toEqual({});
  });

  it("updates passage map for an agent", () => {
    let state = createEmptyState();
    state = addAgentToState(state, "my-app", "agent-123", CREATED_AT);
